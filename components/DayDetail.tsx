...
export default function DayDetail({ day }: DayDetailProps) {
  if (!day) {
    return (
      <div className="w-72 flex-none border-l border-border bg-surface flex items-center justify-center">
        <p className="text-sm text-text-3 text-center px-6">Clique em um dia para ver os detalhes</p>
      </div>
    )
  }
  ...
  return (
    <div className="w-72 flex-none border-l border-border bg-surface flex flex-col overflow-y-auto">
      {/* Header */}
      <div className="px-5 py-4 border-b border-border">
        <p className="text-[11px] text-text-3 capitalize">{weekday}</p>
      ...
      <div className="px-4 py-4 border-t border-border flex flex-col gap-2">
        <button className="w-full py-2 px-4 rounded-xl bg-accent text-white text-sm font-medium hover:bg-accent-strong transition-colors">
          Adicionar entrada
        </button>
        <button className="w-full py-2 px-4 rounded-xl border border-border text-sm font-medium text-text-2 hover:bg-surface-2 transition-colors">
          Adicionar saída
        </button>
        <button className="w-full py-2 px-4 rounded-xl text-sm font-medium text-text-3 hover:text-text-2 transition-colors">
          Ver lançamentos
        </button>
      </div>
    </div>
